...
**Versión 1.18** · Documento de trabajo · Agosto 2026
...
● **género nombrado en los saberes del curso**, además de sostenido por su criterio de evaluación · ○ **sostenido por el criterio de evaluación del curso, pero no nombrado en sus saberes**: incipiente al principio de la secuencia (argumentativo en 2.º ESO), residual al final, cuando el foco del curso se ha desplazado al otro género (expositivo en 2.º Bach), o simplemente ausente del bloque de saberes de ese curso concreto (los dos géneros en 2.º ESO)
...
13. **`saber_vehiculo` no se verifica contra la fuente** — abierta el 2026-08-06 al resolver la 12, que es el primer caso conocido de cita fabricada que ha sobrevivido a las dos pruebas obligatorias. `verificar_derivacion.py` comprueba literalidad de `criterio_oficial.cita` y de lo entrecomillado en §4.3 y §5.4, pero `saber_vehiculo` es texto libre que nadie contrasta. Extenderlo no es mecánico: los `saber_vehiculo` de los packs no son citas literales sino adaptaciones en minúscula y troceadas (*"Análisis de las propiedades textuales: coherencia, cohesión y adecuación"* aparece como tres entradas distintas), así que una comprobación por subcadena exacta daría falsos positivos en masa. Antes de escribir la regla hay que decidir la forma de comparación —y, por el corolario de simetría de CLAUDE.md, **si la regla candidata invalida packs ya validados, la rota es la regla**. Conviene medirlo sobre los tres packs antes de implementarla.
