...
**Versión 1.50** · Documento de trabajo · Septiembre 2026
...
| Versión | Cambios |
|---|---|
| **1.50** | **La condición de evidencia, revisada sobre los diez packs: dos montajes más que no estaban escritos.** La v1.49 la creó para una sola dimensión (`respuesta_a_preguntas`, oral); esta revisión pregunta si hay otras filas que dependan de algo que la tarea tiene que montar y ningún pack lo dice. Dos candidatas, con la misma prueba que entonces —¿puede la fila imprimirse y calificarse igual aunque el montaje no se dé?—. **`refutacion` (debate, 2.º y 4.º ESO)**: sus cuatro niveles piden rebatir «el argumento contrario», y eso no lo garantiza que el debate tenga turnos —los tiene siempre, por definición—, sino que tenga una **réplica**: una fase en la que el otro equipo responde punto por punto, no dos exposiciones paralelas que nunca se tocan. Las otras tres dimensiones del pack (`escucha_activa`, `postura_y_prueba`, `turno_y_cortesia`) quedan fuera: se sostienen solas en cuanto hay más de una intervención, que es lo mínimo que un debate ya garantiza. **`soporte_multimodal` (investigación, los cinco cursos)**: sus descriptores miden el reparto entre texto e imagen **por pantalla**, y sin una herramienta de presentación digital elegida de antemano —no un documento de texto corrido— no hay pantallas que repartir; es la dimensión con más peso del pack (35 % en 2.º ESO) y obligatoria en los cinco cursos, así que el riesgo de fila en blanco era mayor que el de `respuesta_a_preguntas`, no menor. **Siete criterios nuevos con el campo, cero cambios de cálculo**: `condicion_de_evidencia` no toca matrices ni pesos, así que `simular_correccion.py --resumen` sale igual de limpio en los dos packs. `lcl-murcia-debate` pasa a v0.2.0, `lcl-murcia-investigacion` a v0.3.0; revisiones docentes regeneradas. Las 14 comprobaciones, limpias. |
| **1.49** | **La condición de evidencia: un campo de pack para lo que la tarea tiene que montar antes de poder puntuar una fila.** La dimensión `respuesta_a_preguntas` del pack de oral (v1.47) evalúa algo que no depende solo del alumno: si nadie pregunta, no hay respuesta que observar, y la fila se imprime y se califica igual. **La pregunta era dónde vive esa condición**, y había dos candidatos con respuesta clara. La guía del docente queda descartada: `scripts/generar_manual.py` documenta la *aplicación* —tipos de tarea, cursos, puertas, recuentos— y no conoce dimensiones; una condición escrita allí no viaja con el pack, y un pack se distribuye solo. `evalua_proceso` y la puerta de §8 tampoco sirven: eligen instrumento y premarcado, no describen el montaje. Así que **campo nuevo de criterio**, `condicion_de_evidencia` (texto opcional), en `data/esquema-pack.json`, declarado en los seis cursos de la dimensión. **Se llama de evidencia y no «de aplicación» a propósito**: en este diseño ya hay una *puerta de aplicabilidad* (§8) que elige instrumento y una *condición mínima* (§6.4) que toca la nota, y esto no es ninguna de las dos —no cambia el cálculo, no desmarca nada, no esconde la fila—. **Se imprime en la rúbrica analítica y en ningún otro sitio** (§7.1): es una instrucción de montaje para quien califica, no algo que se le pida al alumno, así que no entra en su ficha —hay una prueba en `test/premarcado.mjs` que lo fija en las dos direcciones—. **La comprobación, en los dos validadores**: es una condición de la dimensión, no del curso, así que declararla en unos cursos y en otros no es **error** (`condicion_de_evidencia`), igual que declararla vacía; el texto sí puede variar de un curso a otro, porque quién modera el turno puede cambiar. Entra con su pack trampa en `comprobar_paridad.py` (5 packs) y cuatro casos en `test/validar-reglas.mjs`, con su control de falso positivo. Sobre los diez packs actuales la regla no dice nada, que es lo esperado: la comprobación no es que salte hoy, es que a partir de ahora una condición no pueda quedarse escrita en un solo curso. `lcl-murcia-oral` pasa a v0.5.0; revisiones docentes regeneradas —la condición se lee ahora también ahí y en `ver.py criterio`—. Las 14 comprobaciones, limpias. |
| **1.48** | **La ruta del Marco Teórico deja de ser una ruta y pasa a ser una búsqueda.** La reestructuración del proyecto de Lengua puso `comprobar_todo.py` en rojo entero por un archivo que seguía existiendo un nivel más arriba, y con un mensaje que no se parecía a la avería («cita sin fuente literal», no «no encuentro el archivo»). `verificar_derivacion.py` prueba ahora candidatas —`RUBRICAS_MARCO_TEORICO`, la distribución actual, la anterior y una búsqueda por nombre dentro del repositorio hermano—, deriva la ruta del directorio hermano en vez de llevar `C:\Users\Usuario` dentro del código, y el aviso dice dónde ha mirado. |
