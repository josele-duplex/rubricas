...
**Versión 1.46** · Documento de trabajo · Septiembre 2026
...
| Versión | Cambios |
|---|---|
| **1.46** | **Décimo tipo de tarea: el debate, la mitad de la oralidad que faltaba.** El proyecto de Lengua comprobó antes de construir sus rúbricas de oralidad y encontró aquí la de presentación (`lcl-murcia-oral`, criterio 3.1) pero no la de debate: el **3.2 estaba LIBRE en los seis cursos** y «debate» no existía como tipo de tarea. Alta de `data/pack-lcl-debate.json` (v0.1.0, 11 criterios en 1.º, 2.º y 4.º ESO) sin tocar código, con cuatro dimensiones trasladadas de las rúbricas del Taller de Argumentación de aquel proyecto. 4.º ESO es la única celda ● de la fila (*«La deliberación oral argumentada»* está en sus saberes); 1.º ESO va con tres dimensiones porque su criterio pide escucha activa y aún no refutación. Primer pack con `razon_peso` que declara reparto **igual**, y sin matrices cuantitativas a propósito: en un debate se observan turnos, no unidades contables. Historia completa en `SDD-cambios.md`. |
| **1.45** | **La guía del docente pasa a ser un derivado más, y el barrido para rehacerla destapa un defecto de concordancia en tres packs.** El `.docx` de `docs/` se escribía en Word y sus cifras se habían separado de `data/`: prometía seis tipos de tarea (hay nueve), 163 criterios (hay 232), diecinueve reglas del validador (hay veintiuna) y la exportación a iDoceo como pendiente cuando lleva funcionando desde la v1.43. Se añade `scripts/generar_manual.py`, con el reparto del SDD: la prosa a mano, los datos leídos de `data/catalogo.json`, `data/derivacion-lcl.json`, los packs, `js/validador.js` y `js/motor.js`. De comprobar lo que la guía iba a afirmar salió «Hasta 1 **faltas**» en la primera banda de ortografía de resumen, comentario y reacción (11 condiciones), la misma falta que la v1.41 corrigió sin barrer el resto; el reconocedor ya admitía el singular en las tres implementaciones, así que el arreglo era del pack y ningún umbral se mueve. Historia completa en `SDD-cambios.md`. |
