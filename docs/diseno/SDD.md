...
**Versión 1.44** · Documento de trabajo · Agosto 2026
...
| Versión | Cambios |
|---|---|
| **1.44** | **Se añade la exportación de la rúbrica en blanco al importador de rúbricas de iDoceo, junto a la del CSV de notas ya construida el mismo día.** El docente pidió que las dos vías convivan y elige el docente por instrumento, no la app por él (decisión §17.19, ampliada dos veces el 2026-08-26). Formato confirmado leyendo la imagen de ejemplo oficial de iDoceo (no está en el texto de su documentación): fila = criterio con «nombre + peso%» en dos líneas, columna = nivel de mayor a menor puntuación con «etiqueta + valor» en dos líneas, celda = descriptor. Los valores de nivel son los mismos `valorNivel` de la escala equilibrada que usa «Calificar» (§6.2), así que la aritmética coincide si se comparan las dos vías. CSV y no XLSX: los dos importadores de iDoceo aceptan CSV, así que ninguna de las dos exportaciones pide una librería binaria externa. Nuevo módulo compartido `js/csv.js` (`escaparCeldaCsv`, `filasACsv`, `descargarCsv`, `nombreMmaaaa`) del que pasan a depender tanto la exportación de notas (`js/calificar.js`) como la nueva de rúbrica (`js/ui.js`: `generarCsvRubricaIdoceo`, `descargarRubricaIdoceo`), para no mantener dos veces la misma regla de cita RFC 4180. Botón «Exportar rúbrica (iDoceo)» junto a «Ajustar» y «Calificar» en la vista previa, con microexplicación propia. Probado con los criterios reales de un pack cargado en el navegador: cabecera y primera columna citadas por el salto de línea interno, pesos y descriptores completos, sin tocar `localStorage`. Las 14 comprobaciones, limpias — no toca ningún pack, matriz ni cita. |
| **1.43** | **La decisión 19 se amplía: «el skill califica» no significaba «solo el skill exporta», y la pregunta del docente lo destapó.** Al confirmar que quería seguir calificando en el instituto sin conexión ni IA, quedó claro que la pantalla «Calificar» (§6.5) ya calificaba dentro de la app en `localStorage`, sin pasar por el skill — la v1.42 daba a entender lo contrario. Investigado además cómo importa iDoceo de verdad: tiene **dos** importadores distintos, uno de rúbricas (matriz de criterios × niveles, para calificar tocando cada celda dentro de iDoceo) y otro general de alumnos (una fila por alumno con la nota ya puesta); confirmado con el docente que el que le sirve es el segundo, porque sigue calificando aquí. Se añade **exportación `.csv` de notas** (`js/calificar.js`: `csvNotas`, `escaparCeldaCsv`, botón «Exportar CSV» junto a la lista de alumnos guardados) — Alumno + nota final del instrumento activo, para ese importador general. El detalle que lo rompe si se pasa por alto: los alumnos se escriben «Apellidos, Nombre», así que casi todas las celdas de la primera columna llevan una coma dentro del propio dato, y se citan con `"…"` (RFC 4180) o el CSV no se puede reabrir bien. Probado en las dos capas: `escaparCeldaCsv` y `csvNotas` con Node y una `localStorage` simulada (nombre con coma y con comillas dentro), y el flujo completo en el navegador real — generar la rúbrica, calificar a `García Ruiz, Elena` con `9.45`, guardar, exportar, y confirmar que el `.csv` resultante trae la fila citada correctamente. Se descarta la otra vía (exportar la matriz de la rúbrica al formato del importador de rúbricas de iDoceo): exigiría calificar dentro de iDoceo y dejaría sin uso la pantalla «Calificar» de este proyecto para ese instrumento. Actualizados con el mismo cambio: §12 (nueva fila `.csv`, retirada la fila `.xlsx` que nunca se construyó), §16.1 (roadmap y desglose de Exportación) y §17.19. Las 14 comprobaciones, limpias — el CSV no toca ningún pack, matriz ni cita. |
| **1.42** | **Las seis decisiones que quedaban abiertas en §17 se cierran todas el mismo día, con el criterio del docente.** No tocan código: son confirmaciones de un valor por defecto ya implementado, o cierres de alcance. Condición mínima (§6.2) desactivada, escala equilibrada (§6.2), tope del detractor en 2 puntos (§6.3) y rúbrica holística descartada por ahora quedan tal como estaban por defecto. Publicación (§16.1) resuelta al comprobar que ya estaba hecha: `josele-duplex/rubricas` es público y GitHub Pages sirve la app en `https://josele-duplex.github.io/rubricas/`. Y el solape con el skill `rubricas-lomloe` (decisión 19, H11/C3 de la matriz digital) se resuelve con la división que recomendaba la matriz: la app construye el instrumento, el skill califica con él y exporta a iDoceo — la app deja de prometer esa exportación en §4.1 y en el roadmap de §16.1. Historia completa en `SDD-cambios.md`. |
