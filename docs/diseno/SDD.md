...
**Versión 1.56** · Documento de trabajo · Septiembre 2026
...
| **1.54** | **«Calificar» pasa a ser una cuadrícula, al modo de iDoceo.** El docente calificó una rúbrica y la pantalla le pareció lenta e incómoda: era una columna de tarjetas —un desplegable por componente, cuatro radios por dimensión sin matriz— con la nota al final, y con seis dimensiones había que desplazarse dos o tres pantallas entre marcar y ver el resultado. Ahora es la misma tabla de la rúbrica con las celdas de descriptor pinchables: un clic por fila y la nota, en una barra fija arriba, se recalcula al instante. En las 189 dimensiones con matriz cuantitativa (de 249) el descriptor también se puede pinchar —hasta ahora la app obligaba a marcar la banda de cada componente— y «Contar» despliega dentro de la fila los componentes con sus bandas, también celdas, para cuando se quiere la precisión de §6.3. Cambio de diseño en §6.5: un criterio con matriz produce `nivel` o `matriz` según lo que haga el docente; §6.2 no lo prohibía (prohíbe colapsar puntos contados a nivel), la guía del docente ya lo describía con un «o», y el CSV del importador de rúbricas de iDoceo ya aplanaba todo a cuatro niveles. Estado de fila y su traducción a `ResultadoCriterio` como funciones puras en `js/calificacion.js`, con siete casos nuevos. El formato guardado no cambia. «Guardar» pasa a limpiar la cuadrícula y a dejar el foco en el nombre del siguiente. De paso, la tarjeta de Calificar y la de Ajustar llevaban `margin: 0 auto` dentro de una columna flex, que impedía que se estiraran y hacía desbordar la página en un móvil: fuera. |
